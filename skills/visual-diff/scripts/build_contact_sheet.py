#!/usr/bin/env python3
"""Build a self-contained HTML visual diff contact sheet from a JSON manifest."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("comparison", "after-only"), help="Override manifest mode.")
    parser.add_argument("--title", help="Override page title.")
    parser.add_argument("--subtitle", help="Override page subtitle.")
    parser.add_argument("--before-label", help="Override before column label.")
    parser.add_argument("--after-label", help="Override after column label.")
    parser.add_argument(
        "--min-short-edge",
        type=int,
    ...
    parser.add_argument("--allow-low-res", action="store_true", help="Allow images below the minimum pixel dimensions.")
    parser.add_argument("--allow-lossy", action="store_true", help="Allow lossy screenshot formats such as JPEG.")
...
def render_empty(message: str) -> str:
    return (
        '          <div class="empty">\n'
        "            <div>\n"
        '              <strong>No matching screen</strong>\n'
        f"              {escape(message)}\n"
        "            </div>\n"
        "          </div>"
    )
...
def render_image(src: str, alt: str, image_info: ImageInfo) -> str:
    quality_label = f"{image_info.dimensions_label} {image_info.format_label}"
    return (
        '<div class="shot-block">'
        f'<div class="shot-wrap"><img src="{escape(src)}" alt="{escape(alt)}" loading="lazy"></div>'
        f'<div class="quality-badge">{escape(quality_label)}</div>'
        "</div>"
    )
...
def comparison_section(
    index: int,
    screen: dict[str, Any],
    before_image: tuple[str, ImageInfo] | None,
    after_image: tuple[str, ImageInfo] | None,
    before_label: str,
    after_label: str,
) -> str:
    ...
    before_body = (
        render_image(before_image[0], f"{before_label} {title}", before_image[1])
        if before_image
        else render_empty(str(screen.get("empty_before") or "This screen is new in the current flow."))
    )
    after_body = (
        render_image(after_image[0], f"{after_label} {title}", after_image[1])
        if after_image
        else render_empty(str(screen.get("empty_after") or "This screen was removed from the current flow."))
    )
    note_html = f"        <span>{escape(note)}</span>\n" if note else ""
...
{note_html}      </div>
      <div class="pair">
        <div class="cell">
          <h3>{escape(before_label)}</h3>
{before_body}
        </div>
        <div class="cell">
          <h3>{escape(after_label)}</h3>
{after_body}
        </div>
      </div>
    </section>"""
...
def after_only_section(index: int, screen: dict[str, Any], image_src: str, image_info: ImageInfo) -> str:
    title = str(screen.get("title") or f"Screen {index}")
    note = screen.get("note")
    note_html = f'<p class="screen-note">{escape(note)}</p>' if note else ""
    quality_label = f"{image_info.dimensions_label} {image_info.format_label}"
    return f"""      <section class="screen">
        <div class="screen-header">
          ...
          <div class="screen-badges">
            <span class="quality-badge">{escape(quality_label)}</span>
            <span class="screen-number">{index:02d}</span>
          </div>
        </div>
        <div class="shot-wrap"><img src="{escape(image_src)}" alt="{escape(title)}" loading="lazy"></div>
      </section>"""
...
def main() -> int:
    ...
    mode = args.mode or str(manifest.get("mode") or "comparison")
    if mode not in {"comparison", "after-only"}:
        raise ValueError("Mode must be 'comparison' or 'after-only'.")

    title = args.title or str(manifest.get("title") or "Visual Diff")
    subtitle = args.subtitle if args.subtitle is not None else str(manifest.get("subtitle") or "")
    before_label = args.before_label or str(manifest.get("before_label") or "Before")
    after_label = args.after_label or str(manifest.get("after_label") or "After")
    ...
    for index, raw_screen in enumerate(manifest["screens"], start=1):
        ...
        screen = raw_screen
        screen_title = str(screen.get("title") or f"Screen {index}")
        if mode == "comparison":
            before_image = copy_image(screen.get("before"), manifest_dir, assets_dir, index, "before", screen_title)
            after_image = copy_image(screen.get("after"), manifest_dir, assets_dir, index, "after", screen_title)
            if not before_image and not after_image:
                raise ValueError(f"Screen {index} has neither 'before' nor 'after'.")
            for side, source_value, image in (
                (before_label, screen.get("before"), before_image),
                (after_label, screen.get("after"), after_image),
            ):
                if not image or not source_value:
                    continue
                failures.extend(
                    quality_failures(
                        image_path=str(source_value),
                        image_info=image[1],
                ...
                image_summaries.append(
                    f"{index:02d} {side} {screen_title}: {image[1].dimensions_label} {image[1].format_label}"
                )
            sections.append(comparison_section(index, screen, before_image, after_image, before_label, after_label))
        else:
            image_value = screen.get("image") or screen.get("after")
            image = copy_image(image_value, manifest_dir, assets_dir, index, "after", screen_title)
            if not image:
                raise ValueError(f"Screen {index} must contain 'image' or 'after' in after-only mode.")
            failures.extend(
                quality_failures(
            ...
            image_summaries.append(f"{index:02d} {screen_title}: {image[1].dimensions_label} {image[1].format_label}")
            sections.append(after_only_section(index, screen, image[0], image[1]))
